...
DO $$ BEGIN
    IF NOT EXISTS (
        ...
        WHERE c.relname = 'idx_venue_name' AND n.nspname = 'program'
    ) THEN
...
DO $$ BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_class c JOIN pg_namespace n ON c.relnamespace = n.oid
        WHERE c.relname = 'idx_section_venue_id' AND n.nspname = 'program'
    ) THEN
...
DO $$ BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_class c JOIN pg_namespace n ON c.relnamespace = n.oid
        WHERE c.relname = 'idx_venue_seat_section_id' AND n.nspname = 'program'
    ) THEN
CREATE INDEX idx_venue_seat_section_id ON p_venue_seat (section_id) WHERE deleted_at IS NULL;
END IF;
    IF NOT EXISTS (
        SELECT 1 FROM pg_class c JOIN pg_namespace n ON c.relnamespace = n.oid
        WHERE c.relname = 'idx_venue_seat_physical_status' AND n.nspname = 'program'
    ) THEN
